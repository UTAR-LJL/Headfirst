...
public class GreedyThree {
	...
	public GreedyThree(int stackSum,int orderSum) { 
		...
		for (int i = 0; i < order.size(); i++) {	
			if (order.get(i).orderId == id) {
				...
				if (order.get(i).task.size() == 0) {
					...
					GreedyThree.maxDelayOrder = Math.max(GreedyThree.maxDelayOrder, k);
					totalDelay += k;
...
//			System.out.println("");
			...
			for (int i = 0; i < order.size(); i++) {  //清空 order里面订单的orderTag信息
				...
				o.orderTag = 0;
				o.visited = false;
				o.waitingTime = newOrder.arrivalTime - o.arrivalTime;
		...
		while(true) {
			...
			for (int i = 0; i < order.size(); i++) {
				if ( (order.get(i).visited == false) && (order.get(i).waitingTime >= threshold)  ) {
					
					order.get(i).visited = true;
					handle(order.get(i));
					nextRound = true;
				}		
			}
